...
export default function ChatPage() {
  ...
  const initializeChat = async () => {
    try {
      ...
      let adminChat = chats.find(
        (chat) =>
          chat.type === "USER_ADMIN" && chat.module === "PASSION_SINGLES",
      );

      if (!adminChat) {
        // Create new chat (this would typically be done by backend)
        // For now, we'll show a message
        toast.success("Starting new chat with admin...");
      } else {
  ...
  const handleSendMessage = async () => {
    ...
    setIsTyping(false);
    if (socket) emitTyping(chatId, false);

    try {
      // Send via socket if available, otherwise via HTTP
      if (socket) {
  ...
  const handleTyping = (value: string) => {
    ...
    if (!isTyping) {
      setIsTyping(true);
      if (socket && chatId) emitTyping(chatId, true);
    }
